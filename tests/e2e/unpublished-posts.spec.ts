...
test("los posts en draft no se sirven en la URL de detalle", async ({
  ...
  const response = await request.get("/es/blog/funcional/monadas/");
...
test("el índice público y la API de búsqueda no exponen drafts", async ({
  ...
  expect(docs.some((doc) => doc.id === "es/funcional/monadas")).toBe(false);
  ...
  expect(results.some((doc) => doc.id === "es/funcional/monadas")).toBe(false);
});
