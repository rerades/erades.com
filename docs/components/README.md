...
| Componente | Descripción | Props |
| --- | --- | --- |
| [Base Head](./base-head.md) | Todo lo que va en el <head> de cualquier página: metadatos globales, canónica, Open Graph, Twitter Card, los tres feeds RSS y Google Analytics. | 5 |
...
| [Google Analytics](./google-analytics.md) | Carga GA4 solo si `PUBLIC_GA_MEASUREMENT_ID` está configurada: sin variable no emite ni una etiqueta. `gtag.js` se pide después del evento `load`, para no competir con el render ni con la imagen del LCP, y va con la IP anonimizada y las señales publicitarias desactivadas. El condicional es un `ShowWhen` y no una expresión JSX envolvente por un motivo que está comentado en el fichero: dentro de una expresión, Astro emite el cuerpo del script literal en vez de evaluarlo. | 0 |
...
| [View Transitions](./view-transitions.md) | El pegamento de las transiciones entre páginas: marca el título de la tarjeta que se acaba de pulsar para que vuele hasta el título del post, y decide si la navegación avanza o retrocede para que la animación vaya en el sentido correcto. No renderiza nada; las animaciones en sí viven en `global.css`. Va en el `<head>`, dentro de `BaseHead`. | 0 |
